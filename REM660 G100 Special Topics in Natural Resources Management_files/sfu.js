...
/*
    ...
    ****** TRANSPILE THIS FILE WITH BABEL PRIOR TO UPLOADING TO CANVAS BECAUSE IE 11 ******
    yarn install && npx babel --presets=@instructure/ui-babel-preset public/sfu/js/sfu.js -o /tmp/sfu.js
...
(function ($) {
  const utils = {
    onPage(regex, fn) {
      if (window.location.pathname.match(regex)) fn();
    },

    hasAnyRole() {
      const roles = [].slice.call(arguments, 0);
      const cb = roles.pop();
  ...
  utils.onPage(/^\/(?:courses\/?)?$/, () => {
    utils.onElementRendered('#start_new_course', () => {
      ...
      newCourseButton.setAttribute('href', '/sfu/course/new');
      newCourseButton.innerHTML = 'Start a New Course';
  ...
  utils.onPage(/^\/profile\/settings\/?$/, () => {
    $(document).ready(() => {
      const $fieldsToLock = $('.full_name.display_data, .sortable_name.display_data');
      const $helpText = $('.short_name').siblings('span.edit_or_show_data'); // CANVAS-253 Temporarily make full/sortable names read-only

      $fieldsToLock.removeClass('display_data').addClass('edit_or_show_data');
      $fieldsToLock.siblings('input').hide(); // CANVAS-254 Add verbiage about Display Name

      $helpText.append('<br />Changing this will only affect your display name within Canvas, ' + 'and not in other systems (e.g. <a href="https://go.sfu.ca" target="_blank">goSFU</a>, ' + '<a href="https://myinfo.sfu.ca" target="_blank">myInfo</a>, etc.)'); // CANVAS-259 Hide email subscription checkbox

      $('#update_profile_form').find('label[for="user_subscribe_to_emails"]').parents('tr').hide();
    });
  }); // CANVAS-259 Hide email subscription checkbox

  utils.onPage(/^\/register\/\w+/, () => {
    $(document).ready(() => {
      $('#registration_confirmation_form').find('label[for="user_subscribe_to_emails"]').parents('.control-group').hide();
    });
  }); // END CANVAS-259
  // Remove "Student Annotation" submission type option

  utils.onPage(/^\/courses\/\d+\/assignments\/new/, () => {
    let removeStudentAnnotationTimer = window.setInterval(() => {
      const $target = $('[for="assignment_annotated_document"]');
      console.log($target);

      if ($target.length > 0) {
        $target.remove();
        window.clearInterval(removeStudentAnnotationTimer);
      }
    }, 1000);
...
if (window.location.hostname && window.location.hostname === 'canvas.sfu.ca') {
  ...
  gtag('js', new Date());
  gtag('config', 'G-NXY3EK2M2F');

  (function () {
    const ga = document.createElement('script');
    ga.async = true;
    ga.src = 'https://www.googletagmanager.com/gtag/js?id=G-NXY3EK2M2F';
